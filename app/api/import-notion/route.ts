  ...
  try {
    ...
    if (!pageId)
      return Response.json(
        {
          error:
            '노션 페이지 ID를 찾지 못했어요. 공유 링크를 다시 확인해 주세요.',
        },
        { status: 400 },
      );
    ...
    if (!databaseBlocks.length) {
      return Response.json(
        {
          error:
            '공개된 표 데이터베이스를 찾지 못했어요. 노션 페이지를 웹에 게시했는지 확인해 주세요.',
        },
        { status: 422 },
      );
    }
    ...
    const discussionParents = new Map<
      string,
      { rowId: string; spaceId: string }
    >();
    const viewNames = new Set<string>();
    for (const databaseBlock of databaseBlocks.slice(0, 5)) {
      const collection = collections.find(
        (item) => item.id === databaseBlock.collection_id,
      );
      ...
      for (const viewId of (databaseBlock.view_ids as string[]).slice(0, 12)) {
        ...
        const result = await notionPost(
          'queryCollection',
          {
            ...
            collectionViewBlock: {
              id: databaseBlock.id,
              spaceId: databaseBlock.space_id,
            },
        ...
        for (const row of recordValues(result.recordMap?.block)) {
          if (
            row.type !== 'page' ||
            row.parent_id !== databaseBlock.collection_id ||
            drafts.has(row.id)
          )
            continue;
          ...
          if (draft.title && draft.performanceAt) {
            ...
            for (const discussionId of idList(row.discussions))
              discussionParents.set(discussionId, {
                rowId: row.id,
                spaceId: row.space_id || databaseBlock.space_id || '',
              });
          }
        }
      }
    ...
    try {
      ...
      for (const [rowId, reviews] of reviewsByRow) {
        ...
        if (draft) {
          ...
          commentsImported += reviews.length;
        }
      }
    } catch {
      /* 공연 기본 정보는 유지하고 댓글만 경고로 안내해요. */
    }

    const concerts = [...drafts.values()].sort((a, b) =>
      b.performanceAt.localeCompare(a.performanceAt),
    );
    if (!concerts.length)
      return Response.json(
        { error: '공연명과 날짜가 있는 행을 찾지 못했어요.' },
        { status: 422 },
      );
    return Response.json({
      pageTitle:
        richText(
          propertyOf(
            blocks.find((block) => block.id === pageId),
            'title',
          ),
        ) || 'Notion 공연 기록',
      views: [...viewNames],
      ...
      commentsImported,
      warnings: [
        ...(discussionParents.size > 0 && commentsImported === 0
          ? ['댓글을 읽지 못했어요. 페이지 댓글 공개 설정을 확인해 주세요.']
          : []),
        '비공개 파일 이미지는 자동 이관되지 않아요.',
        '공연장 좌표는 가져온 뒤 공연 상세에서 위치를 확인해 주세요.',
      ],
    });
  } catch (error) {
    const message =
      error instanceof Error ? error.message : '노션 기록을 불러오지 못했어요.';
    return Response.json(
      { error: message },
      { status: message.includes('공개') ? 403 : 500 },
    );
  }
}
...
function parseNotionUrl(raw: string) {
  ...
  try {
    ...
    throw new Error('올바른 노션 링크를 입력해 주세요.');
  }
  if (
    url.protocol !== 'https:' ||
    ...
    throw new Error(
      'notion.so, notion.site 또는 app.notion.com 링크만 사용할 수 있어요.',
    );
  return url;
}
...
async function notionPost(endpoint: string, body: unknown, spaceId?: string) {
  ...
  try {
    ...
    if (response.status >= 300 && response.status < 400)
      throw new Error('노션 요청이 다른 주소로 이동되어 중단했어요.');
    if (!response.ok) {
      if (response.status === 401 || response.status === 403)
        throw new Error(
          '공개되지 않은 노션 페이지예요. 노션에서 웹 게시를 켠 뒤 다시 시도해 주세요.',
        );
      throw new Error('노션이 응답하지 않았어요. 잠시 후 다시 시도해 주세요.');
    }
    const text = await response.text();
    if (text.length > MAX_RESPONSE_BYTES)
      throw new Error('노션 기록이 너무 커서 한 번에 가져올 수 없어요.');
...
function recordValues(table: unknown): NotionRecord[] {
  ...
  return Object.values(table as Record<string, unknown>)
    .map(unwrapRecord)
    .filter((value): value is NotionRecord => value !== null);
}
...
function unwrapRecord(entry: unknown): NotionRecord | null {
  ...
  for (
    let depth = 0;
    depth < 4 && isRecord(value) && isRecord(value.value);
    depth += 1
  )
    value = value.value;
...
function rowToDraft(
  ...
  const byNames = (names: string[]) => {
    const id = Object.keys(schema).find((key) =>
      names.some(
        (name) => normalize(schema[key]?.name || '') === normalize(name),
      ),
    );
  ...
  return {
    ...
    artists: splitTags(
      richText(byNames(['누구야', '아티스트', '공연자', 'Artist', 'Artists'])),
    ),
    ...
    paidAmount: numberValue(
      byNames(['수수료 포함', '실제 결제액', '결제액', 'Paid', 'Amount']),
    ),
    reviews: [],
  };
}
...
async function loadReviews(
  discussionParents: Map<string, { rowId: string; spaceId: string }>,
) {
  const discussions = await syncRecords(
    'discussion',
    [...discussionParents.keys()],
    discussionParents,
  );
  ...
  for (const discussion of discussions) {
    ...
    for (const commentId of discussion.comments || [])
      commentParents.set(commentId, parent);
  }
  const comments = await syncRecords(
    'comment',
    [...commentParents.keys()],
    commentParents,
  );
  ...
  for (const comment of comments) {
    ...
    const review: ConcertReview = {
      ...
      body,
      createdAt:
        typeof comment.created_time === 'number'
          ? new Date(comment.created_time).toISOString()
          : new Date().toISOString(),
    };
    reviews.set(parent.rowId, [...(reviews.get(parent.rowId) || []), review]);
  }
  for (const items of reviews.values())
...
async function syncRecords(
  ...
  ids: string[],
  parents: Map<string, { rowId: string; spaceId: string }>,
) {
  ...
  for (let index = 0; index < ids.length; index += 100) {
    ...
    const result = await notionPost(
      'syncRecordValues',
      { requests: batch.map((id) => ({ table, id, version: -1 })) },
      spaceId,
    );
...
function propertyOf(block: NotionRecord | undefined, id: string) {
...
function idList(value: string[] | string | undefined) {
...
function notionDate(value: unknown): { start: string; end?: string } {
  if (
    ...
    return { start: '' };
  const datePair = value[0][1].find(
    (item: unknown) => Array.isArray(item) && item[0] === 'd',
  );
  const metadata =
    Array.isArray(datePair) && isRecord(datePair[1]) ? datePair[1] : undefined;
  return {
    start: typeof metadata?.start_date === 'string' ? metadata.start_date : '',
    end: typeof metadata?.end_date === 'string' ? metadata.end_date : undefined,
  };
}
