...
export async function POST(request: Request) {
  ...
  if (
    ...
    return Response.json(
      { error: 'NOL 티켓, YES24 티켓, 멜론티켓의 HTTPS 주소만 지원해요.' },
      { status: 400 },
    );
  ...
  try {
    ...
    if (nolGoodsCode)
      return Response.json(
        await importNolConcert(nolGoodsCode, url, controller.signal),
      );
    const melonProdId = getMelonProdId(url);
    if (melonProdId)
      return Response.json(
        await importMelonConcert(melonProdId, url, controller.signal),
      );
    ...
    for (let redirects = 0; redirects < 4; redirects += 1) {
      response = await fetch(url, {
        ...
        headers: { 'user-agent': 'DukjilLog/1.0 (+concert metadata import)' },
      });
    ...
    if (!(response.headers.get('content-type') || '').includes('text/html'))
      return Response.json(
        { error: 'HTML 공연 페이지만 불러올 수 있어요.' },
        { status: 415 },
      );
    const length = Number(response.headers.get('content-length') || 0);
    if (length > 2_000_000)
      return Response.json(
        { error: '페이지가 너무 커서 자동으로 불러올 수 없어요.' },
        { status: 413 },
      );
    const html = (await response.text()).slice(0, 2_000_000);
    const metadata = url.hostname.toLowerCase().includes('yes24.com')
      ? extractYes24Metadata(html, url)
    ...
    return Response.json(
      {
        error:
          '예매처가 자동 불러오기를 허용하지 않거나 페이지가 응답하지 않았어요.',
      },
      { status: 502 },
    );
  } finally {
...
function extractYes24Metadata(html: string, url: URL) {
  ...
  const field = (label: string) =>
    stripTags(
      matchFirst(
        html,
        new RegExp(
          `<dt[^>]*>\\s*${escapeRegex(label)}\\s*</dt>\\s*<dd[^>]*>([\\s\\S]*?)</dd>`,
          'i',
        ),
      ),
    );
  const title =
    ...
    decode(
      matchFirst(
        html,
        /<em[^>]+class=["'][^"']*gd_name[^"']*["'][^>]*>([\s\S]*?)<\/em>/i,
      ),
    );
  const period =
    field('일시') ||
    meta('description').match(
      /일시:\s*(20\d{2}[.\/-]\d{1,2}[.\/-]\d{1,2}\s*~\s*20\d{2}[.\/-]\d{1,2}[.\/-]\d{1,2})/,
    )?.[1] ||
    '';
  ...
  const dateCandidates = range
    ? expandKoreanDateRange(
        `${range[1]}-${range[2].padStart(2, '0')}-${range[3].padStart(2, '0')}`,
        `${range[4]}-${range[5].padStart(2, '0')}-${range[6].padStart(2, '0')}`,
      )
    : single
      ? [
          `${single[1]}-${single[2].padStart(2, '0')}-${single[3].padStart(2, '0')}T00:00:00+09:00`,
        ]
      : [];
  const priceSection = matchFirst(
    html,
    /<dl[^>]+class=["'][^"']*gd_priceDl[^"']*["'][^>]*>([\s\S]*?)<\/dl>/i,
  );
  const priceCandidates = [
    ...new Set(
      [
        ...stripTags(priceSection).matchAll(/([1-9]\d{0,2}(?:,\d{3})+)\s*원/g),
      ].map((match) => Number(match[1].replaceAll(',', ''))),
    ),
  ].filter((value) => value >= 1000 && value <= 10_000_000);
  const leadRole = decode(
    matchFirst(html, /id=["']HidLeadRole["'][^>]+value=["']([^"']*)["']/i) ||
      matchFirst(html, /value=["']([^"']*)["'][^>]+id=["']HidLeadRole["']/i),
  );
  const artists =
    leadRole && leadRole !== '-'
      ? leadRole
...
function expandKoreanDateRange(start: string, end: string) {
  ...
  for (
    ...
    cursor <= endTime && result.length < 31;
    cursor += 86_400_000
  )
    result.push(
      `${new Date(cursor).toISOString().slice(0, 10)}T00:00:00+09:00`,
    );
  return result;
}
...
function getNolGoodsCode(url: URL) {
  ...
  return (
    url.pathname.match(/\/goods\/(\d{8})(?:\/|$)/i)?.[1] ||
    url.searchParams.get('GoodsCode')?.match(/^\d{8}$/)?.[0] ||
    ''
  );
}
...
function getMelonProdId(url: URL) {
  ...
  const hashQuery = url.hash.includes('?')
    ...
    : '';
  const value =
    url.searchParams.get('prodId') ||
    new URLSearchParams(hashQuery).get('prodId') ||
    '';
  return /^\d{1,10}$/.test(value) ? value : '';
}

async function importMelonConcert(
  prodId: string,
  sourceUrl: URL,
  signal: AbortSignal,
) {
  const detailResponse = await fetchMelonJson(
    `/poc/performance/detail.json?prodId=${encodeURIComponent(prodId)}&v=1`,
    signal,
  );
  ...
  const placeResponse = product.placeId
    ? await fetchMelonJson(
        `/poc/place/detail.json?placeId=${encodeURIComponent(String(product.placeId))}&v=1`,
        signal,
      ).catch(() => null)
    : null;
  const place = placeResponse?.result === 0 ? placeResponse.data : null;
  const hall = Array.isArray(place?.placeHallVoList)
    ? place.placeHallVoList.find(
        (item: any) => Number(item?.hallId) === Number(product.hallId),
      )
    : null;
  const venueBase = decode(place?.name || product.placeName);
  const hallName = decode(hall?.name || product.availPlaceInfo);
  const venue =
    hallName && hallName !== venueBase ? `${venueBase} ${hallName}` : venueBase;
  const artists = parseMelonArtists(product.actorJson);
  const priceCandidates = parseMelonPrices(product);
  const dateCandidates = parseMelonDates(
    product.perfTimeInfo,
    product.periodInfo,
    product.compsVO,
  );
  const warnings: string[] = [];
  ...
  if (dateCandidates.length > 1)
    warnings.push('관람한 공연일을 선택해 주세요.');
  if (!dateCandidates.length) warnings.push('공연 날짜를 직접 확인해 주세요.');
  if (!priceCandidates.length) warnings.push('티켓 가격을 직접 확인해 주세요.');
  return {
...
async function fetchMelonJson(path: string, signal: AbortSignal) {
  const response = await fetch(`https://tktapi.melon.com${path}`, {
    ...
    headers: {
      ...
      'user-agent':
        'Mozilla/5.0 (compatible; DukjilLog/1.0; +concert metadata import)',
    },
  });
  if (
    !response.ok ||
    ...
    throw new Error('Melon API unavailable');
  const length = Number(response.headers.get('content-length') || 0);
...
function parseMelonArtists(value: unknown) {
  try {
    ...
    const list = Array.isArray((parsed as any)?.data?.list)
      ? (parsed as any).data.list
      : [];
    return [
      ...new Set(
        list
          .map((item: any) =>
            decode(item?.artistRepNm || item?.artistNameWebList),
          )
          .filter(Boolean),
      ),
...
function parseMelonPrices(product: any) {
  const values: number[] = [];
  if (Array.isArray(product?.gradelist))
    product.gradelist.forEach((item: any) =>
      values.push(Number(item?.basePrice)),
    );
  try {
    const parsed =
      typeof product?.seatGradeJson === 'string'
        ? JSON.parse(product.seatGradeJson)
        : product?.seatGradeJson;
    if (Array.isArray(parsed?.data?.list))
      parsed.data.list.forEach((item: any) =>
        values.push(Number(item?.basePrice)),
      );
  } catch {
    /* gradelist 값으로 계속 진행해요. */
  }
  return [
    ...new Set(
      values.filter(
        (value) =>
          Number.isFinite(value) && value >= 1000 && value <= 10_000_000,
      ),
    ),
  ]
    .sort((a, b) => a - b)
...
function parseMelonDates(
  perfTimeInfo: unknown,
  periodInfo: unknown,
  comps: any,
) {
  const text = stripTags(String(perfTimeInfo || '')).replace(/\s+/g, ' ');
  const range = text.match(
    /(20\d{2})\s*년\s*(\d{1,2})\s*월\s*(\d{1,2})\s*일(?:\s*\([^)]*\))?\s*~\s*(?:(20\d{2})\s*년\s*)?(\d{1,2})\s*월\s*(\d{1,2})\s*일(?:\s*\([^)]*\))?(?:\s*(오전|오후)?\s*(\d{1,2})\s*시(?:\s*(\d{1,2})\s*분)?)?/,
  );
  if (range) {
    ...
    return expandKoreanDateRange(start, end).map((value) =>
      value.replace(
        'T00:00:00',
  ...
  const explicit = [
    ...text.matchAll(
      /(20\d{2})\s*년\s*(\d{1,2})\s*월\s*(\d{1,2})\s*일(?:\s*\([^)]*\))?(?:\s*(오전|오후)?\s*(\d{1,2})\s*시(?:\s*(\d{1,2})\s*분)?)?/g,
    ),
  ].map((match) => {
  ...
  const period = String(periodInfo || '').match(
    /(20\d{2})[.\/-](\d{1,2})[.\/-](\d{1,2})\s*[-~]\s*(20\d{2})[.\/-](\d{1,2})[.\/-](\d{1,2})/,
  );
  const compactStart = String(comps?.performanceStartDay || '');
  const compactEnd = String(comps?.performanceEndDay || '');
  const start = period
    ? `${period[1]}-${period[2].padStart(2, '0')}-${period[3].padStart(2, '0')}`
    : /^20\d{6}$/.test(compactStart)
      ? `${compactStart.slice(0, 4)}-${compactStart.slice(4, 6)}-${compactStart.slice(6, 8)}`
      : '';
  const end = period
    ? `${period[4]}-${period[5].padStart(2, '0')}-${period[6].padStart(2, '0')}`
    : /^20\d{6}$/.test(compactEnd)
      ? `${compactEnd.slice(0, 4)}-${compactEnd.slice(4, 6)}-${compactEnd.slice(6, 8)}`
      : start;
...
async function importNolConcert(
  goodsCode: string,
  sourceUrl: URL,
  signal: AbortSignal,
) {
  ...
  const [summary, playMetadata] = await Promise.all([
    fetchNolJson(
      `/v1/goods/${goodsCode}/summary?passCode=&seatGrade=&priceGrade=`,
      referer,
      signal,
    ).catch(() => null),
  ...
  const [place, priceGroups, info] = await Promise.all([
    goods.placeCode
      ? fetchNolJson(
          `/v1/Place/${encodeURIComponent(goods.placeCode)}`,
          referer,
          signal,
        ).catch(() => null)
      : Promise.resolve(null),
    fetchNolJson(`/v1/goods/${goodsCode}/prices/group`, referer, signal).catch(
      () => null,
    ),
    fetchNolJson(
      `/v1/goods/${goodsCode}/tab/info?goodsCode=${goodsCode}&topingInclude=false`,
      referer,
      signal,
    ).catch(() => null),
  ]);
  const dateCandidates = parseNolDates(
    goods.playTime,
    goods.playStartDate,
    goods.playEndDate,
  );
  ...
  const posterUrl = absoluteHttpsUrl(
    goods.goodsLargeImageUrl || goods.goodsSmallImageUrl,
  );
  const warnings: string[] = [];
  if (!artists.length) warnings.push('아티스트를 직접 확인해 주세요.');
  ...
  if (!priceCandidates.length)
    warnings.push(
      'NOL 페이지에서 가격을 텍스트로 제공하지 않아 티켓 가격을 직접 확인해 주세요.',
    );
...
async function fetchNolPlayMetadata(goodsCode: string, signal: AbortSignal) {
  const meta = await fetchNolJson(
    `/v1/meta/${goodsCode}/performance?goodsCode=${goodsCode}`,
  ...
  const response = await fetch(
    `https://mobileticket.interpark.com/play/performance/${encodeURIComponent(slug)}`,
    {
      signal,
      headers: {
        accept: 'text/html',
  ...
  const metaValue = (property: string) =>
    decode(
      matchFirst(
        html,
        new RegExp(
  ...
  const title =
    metaValue('og:title').replace(/^NOL 티켓\s*\|\s*/, '') ||
    decode(matchFirst(html, /<title[^>]*>([^<]+)<\/title>/i));
  const venue = decode(
    matchFirst(html, /\\"venueName\\":\\"([^"\\]+)\\"/i) ||
      matchFirst(
        html,
        /<span[^>]*>([^<]+)<\/span><\/li><li><span>20\d{2}[.\/-]/i,
      ),
  );
  const start = matchFirst(
    html,
    /\\"performStartDate\\":\\"(20\d{2}-\d{2}-\d{2})\\"/i,
  );
  const end = matchFirst(
    html,
    /\\"performEndDate\\":\\"(20\d{2}-\d{2}-\d{2})\\"/i,
  );
  const placeCode = matchFirst(
    html,
    /\\"ticket2000PlaceCode\\":\\"([^"\\]+)\\"/i,
  );
  return {
    goodsName: title,
    placeName: venue,
    ...
    playStartDate: start,
    playEndDate: end,
    goodsLargeImageUrl: metaValue('og:image'),
  };
}

async function fetchNolJson(
  path: string,
  referer: string,
  signal: AbortSignal,
) {
  ...
  if (response.status >= 300 && response.status < 400)
    throw new Error('Unexpected NOL redirect');
  if (
    !response.ok ||
    !(response.headers.get('content-type') || '').includes('json')
  )
    throw new Error('NOL API unavailable');
  const length = Number(response.headers.get('content-length') || 0);
  if (length > 1_000_000) throw new Error('NOL response too large');
  return JSON.parse((await response.text()).slice(0, 1_000_000));
}

function parseNolDates(
  playTime: unknown,
  startDate: unknown,
  endDate: unknown,
) {
  ...
  const matches = [
    ...text.matchAll(
      /(20\d{2})년\s*(\d{1,2})월\s*(\d{1,2})일(?:\([^)]*\))?\s*(\d{1,2}):(\d{2})\s*(AM|PM)/gi,
    ),
  ];
  ...
  return start && end
    ? expandKoreanDateRange(start, end)
    : [
        ...new Set(
          [start, end]
            .filter(Boolean)
            .map((value) => `${value}T00:00:00+09:00`),
        ),
      ];
}
...
function collectNolArtists(info: unknown, title: unknown) {
  ...
  const visit = (value: unknown, key = '') => {
    if (
      typeof value === 'string' &&
      /manName|artistName|performerName|castName/i.test(key) &&
      value.trim()
    )
  ...
  visit(info);
  return [...new Set(values.length ? values : inferNolArtists(title))].slice(
    0,
    20,
  );
}
...
function collectNolPrices(priceGroups: any, goods: any, info?: unknown) {
  ...
  const visit = (value: unknown, key = '') => {
    if (typeof value === 'number' && /price/i.test(key)) values.push(value);
      ...
      for (const match of value.matchAll(/[1-9]\d{0,2}(?:,\d{3})+/g))
  ...
  visit(priceGroups);
  visit({
    minPrice: goods.minSalesPrice,
    maxPrice: goods.maxSalesPrice,
    basicPrice: goods.basicPriceHtml,
  });
  visit(info);
  return [
    ...new Set(values.filter((value) => value >= 1000 && value <= 10_000_000)),
  ]
    .sort((a, b) => a - b)
    .slice(0, 8);
}
...
function extractMetadata(html: string, url: URL) {
  ...
  const event = jsonLd.find((item: any) =>
    String(item?.['@type'] || '')
      .toLowerCase()
      .includes('event'),
  ) as any;
  const title = decode(
    ...
    .replace(/\s*[-|｜]\s*(NOL|인터파크|YES24|멜론티켓).*$/i, '')
    .trim();
  ...
  const artists = performers
    .map((item: any) => decode(typeof item === 'string' ? item : item?.name))
    .filter(Boolean);
  ...
  const dates = [
    ...new Set(
      [
        event?.startDate,
        ...[...html.matchAll(/20\d{2}[.\/-]\s?\d{1,2}[.\/-]\s?\d{1,2}/g)].map(
          (match) => match[0],
        ),
      ].filter(Boolean),
    ),
  ...
  const prices = [
    ...new Set(
      [...html.matchAll(/([1-9]\d{1,2}(?:,\d{3})+)\s*원/g)]
        .map((match) => Number(match[1].replaceAll(',', '')))
        .filter((value) => value >= 1000 && value <= 10_000_000),
    ),
  ].slice(0, 8);
  const provider = url.hostname.includes('yes24')
    ? 'YES24 티켓'
    : url.hostname.includes('melon')
      ? '멜론티켓'
      : 'NOL 티켓';
  const eventImage =
    typeof event?.image === 'string'
      ...
      : Array.isArray(event?.image)
        ...
        : event?.image?.url;
  const poster = decode(
    eventImage || meta('og:image') || meta('twitter:image'),
  );
  const warnings = [];
