...
export function RatingPicker({ value, onChange }: { value: number | null; onChange: (value: number | null) => void }) {
  return <div className="rounded-2xl bg-black/[0.035] p-3">
    <div className="flex items-center justify-between gap-3"><span className="text-xs font-medium text-black/55">별점</span>{value == null ? <span className="text-xs text-black/35">미입력</span> : <span className="flex items-center gap-2"><RatingStars value={value} /><button type="button" onClick={() => onChange(null)} className="text-[10px] text-black/35 underline">지우기</button></span>}</div>
    <input aria-label="별점 선택" type="range" min="0.5" max="5" step="0.5" value={value ?? 0.5} onChange={(event) => onChange(Number(event.target.value))} className="mt-3 h-2 w-full cursor-pointer accent-[#ff6b61]" />
