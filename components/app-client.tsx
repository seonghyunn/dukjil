...
function NowView({ concerts, onAdd, onSelect }: { concerts: Concert[]; onAdd: () => void; onSelect: (concert: Concert) => void }) {
  ...
  return <section className="animate-in fade-in pb-20 duration-300">
    <header className="flex items-end justify-between gap-4"><div><p className="eyebrow">Dukjil log</p><h1 className="screen-title">공연 생활, 한눈에</h1><p className="mt-2 text-sm text-white/40">기록이 쌓일수록 나만의 공연 취향이 선명해져요.</p></div><Button size="icon-lg" className="size-11 shrink-0 rounded-full bg-[#ff6b61] text-black hover:bg-[#ff827a]" onClick={onAdd}><Plus /></Button></header>
    ...
    <div className="mt-7 grid gap-4 lg:grid-cols-[1.55fr_.75fr]">
      ...
      <button onClick={() => nextConcert && onSelect(nextConcert)} disabled={!nextConcert} className="group relative min-h-44 overflow-hidden rounded-[30px] border border-white/10 bg-white/[0.045] p-5 text-left disabled:cursor-default">
        ...
        <span className="relative block text-xs text-[#dfff94]">Next stage</span><b className="relative mt-3 block max-w-[72%] text-xl leading-tight">{nextConcert?.title || '예정된 공연이 없어요'}</b><span className="relative mt-3 block max-w-[72%] text-xs leading-5 text-white/45">{nextConcert ? `${dateLabel(nextConcert.performanceAt)} · ${nextConcert.venue}` : '새 공연을 추가해 다음 무대를 기다려 보세요.'}</span>{nextConcert && <ChevronRight className="absolute bottom-5 right-5 size-5 text-white/50 transition group-hover:translate-x-1" />}
      </button>
    </div>
    ...
    <section className="mt-9">
      <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs text-white/35">Concert ledger</p><h2 className="mt-1 text-xl font-semibold">공연냠냠</h2></div><div className="flex rounded-full border border-white/10 bg-white/[0.035] p-1">{([['all', '전체'], ['attended', '관람 완료'], ['scheduled', '예정']] as const).map(([value, label]) => <button key={value} onClick={() => setFilter(value)} className={`rounded-full px-3 py-1.5 text-xs transition ${filter === value ? 'bg-[#dfff94] text-black' : 'text-white/45 hover:text-white'}`}>{label}</button>)}</div></div>
      <div className="mt-4 overflow-hidden rounded-[26px] border border-white/10 bg-white/[0.025]">
        <div className="hidden grid-cols-[76px_1fr_150px_120px] gap-4 border-b border-white/10 px-4 py-3 text-[10px] uppercase tracking-wider text-white/30 sm:grid"><span>날짜</span><span>공연</span><span>장소</span><span className="text-right">결제액</span></div>
        {visibleConcerts.length ? visibleConcerts.map((concert) => <button key={concert.id} onClick={() => onSelect(concert)} className="grid w-full grid-cols-[56px_1fr_auto] items-center gap-3 border-b border-white/[0.07] px-3 py-3 text-left last:border-0 hover:bg-white/[0.045] sm:grid-cols-[60px_1fr_150px_120px] sm:px-4"><span className="text-center"><b className="block text-lg leading-none">{new Date(concert.performanceAt).getDate()}</b><span className="mt-1 block text-[10px] text-white/35">{new Date(concert.performanceAt).getMonth() + 1}월</span></span><span className="flex min-w-0 items-center gap-3"><PosterImage src={concert.posterUrl} title={concert.title} className="size-11 shrink-0 rounded-xl object-cover" /><span className="min-w-0"><b className="block truncate text-sm">{concert.title}</b><span className="mt-1 flex items-center gap-2 text-[11px] text-white/35"><i className={`status-dot ${concert.status}`} />{concert.artists.join(' · ') || '아티스트 미입력'} · {concert.status === 'attended' ? '관람 완료' : '예정'}</span></span></span><span className="hidden truncate text-xs text-white/40 sm:block">{concert.venue}</span><span className="text-right text-xs font-medium text-white/65">{concert.paidAmount == null ? '미입력' : money(concert.paidAmount)}</span></button>) : <div className="p-10 text-center text-sm text-white/35">이 조건에 맞는 공연이 아직 없어요.</div>}
      </div>
    </section>
  </section>;
}
...
function DashboardMetric({ icon, label, value, detail, accent = false }: { icon: React.ReactNode; label: string; value: string; detail: string; accent?: boolean }) { return <div className={`min-w-0 rounded-[24px] border p-4 ${accent ? 'border-[#dfff94]/25 bg-[#dfff94]/10' : 'border-white/10 bg-white/[0.035]'}`}><span className={`grid size-8 place-items-center rounded-xl [&>svg]:size-4 ${accent ? 'bg-[#dfff94] text-black' : 'bg-white/10 text-[#ff847b]'}`}>{icon}</span><p className="mt-4 text-[10px] text-white/35">{label}</p><p className="mt-1 truncate text-lg font-semibold tracking-[-0.035em]">{value}</p><p className="mt-1 truncate text-[10px] text-white/30">{detail}</p></div>; }
