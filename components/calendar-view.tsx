...
export function CalendarView({ concerts, onSelect }: { concerts: Concert[]; onSelect: (concert: Concert) => void }) {
  ...
  return (
    <section className="animate-in fade-in duration-300 pb-28">
      ...
      <div className="mt-4 grid grid-cols-3 gap-2 rounded-2xl bg-white/[0.04] p-1.5">
        {([['all', '전체'], ['scheduled', '예정'], ['attended', '관람 완료']] as const).map(([value, label]) => <button key={value} onClick={() => setFilter(value)} className={`rounded-xl py-2 text-xs transition ${filter === value ? 'bg-white text-black' : 'text-white/45'}`}>{label}</button>)}
      </div>
      <div className="mt-5 grid grid-cols-7 text-center text-[11px] text-white/35">{week.map((day, i) => <span key={day} className={i === 0 ? 'text-[#ff7a70]' : ''}>{day}</span>)}</div>
      <div className="mt-2 grid grid-cols-7 overflow-hidden rounded-3xl border border-white/10 bg-white/[0.025]">
        {cells.map((cell, index) => cell ? (
          <button key={cell.key} onClick={() => setSelectedKey(cell.key)} aria-label={`${cell.key}, 공연 ${cell.concerts.length}개`} className={`relative aspect-[0.82] overflow-hidden border-b border-r border-white/[0.07] text-left ${selectedKey === cell.key ? 'ring-2 ring-inset ring-[#dfff94]' : ''}`}>
            ...
            <span className={`absolute left-1.5 top-1.5 grid size-6 place-items-center rounded-full text-[11px] font-semibold ${cell.concerts[0] ? 'bg-black/55 text-white' : 'text-white/40'}`}>{cell.day}</span>
        ...
        ) : <span key={`empty-${index}`} className="aspect-[0.82] border-b border-r border-white/[0.05]" />)}
      </div>
      <div className="mt-6">
        {selected.length ? <div className="space-y-3">{selected.map((concert) => <button key={concert.id} onClick={() => onSelect(concert)} className="flex w-full items-center gap-4 rounded-3xl border border-white/10 bg-white/[0.04] p-3 text-left"><PosterImage src={concert.posterUrl} title={concert.title} className="size-20 shrink-0 rounded-2xl object-cover" /><span className="min-w-0 flex-1"><b className="block truncate">{concert.title}</b><span className="mt-1 block text-sm text-white/50">{concert.artists.join(' · ')}</span><span className="mt-2 flex items-center gap-1 text-xs text-white/40"><MapPin className="size-3" />{concert.venue}</span></span><span className={`status-dot ${concert.status}`} /></button>)}</div> : <div className="rounded-3xl border border-dashed border-white/10 p-6 text-center text-sm text-white/35">공연이 있는 날짜를 눌러 기억을 펼쳐보세요.</div>}
      </div>
    </section>
  );
}
