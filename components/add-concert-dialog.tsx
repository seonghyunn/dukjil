...
export function AddConcertDialog({ open, onOpenChange, onSave, concerts }: Props) {
  ...
  async function saveNotionConcerts() {
    ...
    const imported: Concert[] = selected.map((draft) => { const location = notionLocations[draft.sourceId]; return ({
      ...
      venue: draft.venue || '장소 미입력', address: location?.address || draft.venue || '', latitude: location?.latitude ?? null, longitude: location?.longitude ?? null, countryCode: location?.countryCode || 'KR',
